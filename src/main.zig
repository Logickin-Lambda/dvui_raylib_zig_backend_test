...
const raylib_backend = @import("backend");
...
pub fn main() !void {
    ...
    raylib_backend.enableRaylibLogging();

    // Create a window from for your operating system
    // all you need is to initialize the windows from the raylib backend
    var backend = try raylib_backend.initWindow(.{
        .gpa = arena.allocator(),
        .size = .{ .w = 1200.0, .h = 900.0 },
        .vsync = false,
        .title = "First Dvui Application Test",
    });
    defer backend.deinit();
    backend.log_events = true;

    // At the current point, dvui is still detached from the os windows, so we need to pass the backend our
    // our choice into the dvui initialization process; however, the use of @src is unknown, and I need to figure
    // out how it works for dvui.
    var win = try dvui.Window.init(@src(), arena.allocator(), backend.backend(), .{
...
fn dvui_frame() bool {
...

    {
        ...
        var box = dvui.box(@src(), .{}, .{ .expand = .horizontal, .min_size_content = .{ .h = 80 }, .background = true, .margin = .{ .x = 8, .w = 8 } });
        defer box.deinit();
        ...
        raylib_direct.drawText("Congrats! You created your first window, loaded raylib directly from the dependency!", @intFromFloat(r.x + 10 * s), @intFromFloat(r.y + 10 * s), @intFromFloat(20 * s), raylib.Color.light_gray);
        raylib.drawText("Congrats! You created your first window, loaded raylib from the dvui raylib backend!", @intFromFloat(r.x + 10 * s), @intFromFloat(r.y + 40 * s), @intFromFloat(20 * s), raylib.Color.light_gray);
    }
